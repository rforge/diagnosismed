...
#' @param type Possible values are "wilson", "exact" and "approximate". See details.
#'
#' @details
#'
#' \code{binom.CI} with type "exact", calculates exact confidence intervals for binomial counts or proportions. This function uses R's \code{\link[stats]{binom.test}} function; however, the arguments to this function can be numeric vectors of any length.
#'
...
#'
#' \code{x} Number of successes in n trials
#'
#' \code{n} Number of Bernoulli trials
#'
#' \code{prop} Proportion = x / n
#'
#' \code{lower} Lower confidence interval limit
#'
#' \code{upper} Upper Confidence interval limit
#'
#' \code{conf.level} Confidence level
#'
#' @references
...
#'
#' @seealso \code{\link[stats]{binom.test}}, \code{\link{SS}}, \code{\link{diagnosis}}
#'
#' @examples
...
#' @export
binom.CI <- function(x, n, conf.level = 0.95, type = c("wilson", "exact",
                                                       "approximate")) {
# warning section
  ...
  binom.wilson <- function(x, n, conf.level = conf.level) {
    ...
    output <- data.frame(x = x, n = n, proportion = x / n, lower = R.lower,
                         upper = R.upper, conf.level = conf.level)
    output
  }
  binom.exact <- function(x, n, conf.level = conf.level) {
    ...
    for (i in 1:nrow(xnc)) {
      ci <- binom.test(x = xnc[i, 1], n = xnc[i, 2], conf.level =
                         xnc[i, 3])$conf.int
      lower[i] <- ci[1]
      upper[i] <- ci[2]
    }
    output <- data.frame(x = x, n = n, proportion = x / n, lower = lower,
  ...
  binom.approx <- function(x, n, conf.level = conf.level) {
    ...
    R.lci <- x / n - Z * SE.R
    R.uci <- x / n + Z * SE.R
    output <- data.frame(x = x, n = n, proportion = x / n, lower = R.lci,
                         upper = R.uci, conf.level = conf.level)
    output
  }
  if (type[1] == "wilson") {
  ...
  if (type[1] == "approximate") {
    output <- binom.approx(x = x, n = n, conf.level = conf.level)
  }
  output
}
