#' @export
print.TGROC <- function(x, ..., digits = 3, type = "all"){
  if(type != "all" && type != "NN-parametric" && type != "Non-parametric" && type != "Binormal"){
    stop("type argument in print.TGROC is not valid.")
  }
  ...
  cat("\n\n")
  if(any(type %in% c("all" , "Non-parametric"))){
  ...
  if(any(type %in% c("all" , "NN-parametric"))){
