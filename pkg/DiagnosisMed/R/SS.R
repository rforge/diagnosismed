#test <- pop1$test ; length(test)
#gold <- pop1$reference ; length(gold)


SS <- function(ref, test, reverse = "auto", CL = 0.95){
  # Warning section ...
  ...
  test.diag.table$Sensitivity <- test.diag.table$TP/D
  ...
  test.diag.table$Specificity <- test.diag.table$TN/ND
  ...
  test.diag.table$PPV <- test.diag.table$TP / (test.diag.table$TP + test.diag.table$FP)
  ...
  #names(output) <-c('test.diag.table') 
  invisible(output)
}
